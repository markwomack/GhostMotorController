...
void setupCallback(void) {
  ...
  attachInterrupt(U_ENCODER_SIGNAL_PIN, countUTick, RISING);
  
  pinMode(V_ENCODER_SIGNAL_PIN, INPUT);
  attachInterrupt(V_ENCODER_SIGNAL_PIN, countVTick, RISING);
  
  pinMode(W_ENCODER_SIGNAL_PIN, INPUT);
  attachInterrupt(W_ENCODER_SIGNAL_PIN, countWTick, RISING);
}
...
void startCallback(void) {
  ...
  taskManager.callbackEvery(100, printCounts, (void*)0);
