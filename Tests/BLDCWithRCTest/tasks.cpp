...
void AdjustMotorSpeedsTask::start(void) {
  ...
  DebugMsgs.debug().println("setup complete");
}

void AdjustMotorSpeedsTask::update(void) {
  int newM1Speed = _motor1.motorEncoderInfo->speed;
  int rc1Value = _rcChannel1->getValue();
  // DebugMsgs.debug().print("Channel 1 value: ").println(rc1Value);

  if (rc1Value != NO_RC_SIGNAL) {
    int motor1TargetSpeed = (int)(rc1Value * MOTOR_INCREMENT);
  ...
  int newM2Speed = _motor2.motorEncoderInfo->speed;
  int rc2Value = _rcChannel1->getValue(); //**** use channel 1 for both motors for now
  // DebugMsgs.debug().print("Channel 2 value: ").println(rc2Value);

  if (rc2Value != NO_RC_SIGNAL) {
    int motor2TargetSpeed = (int)(rc2Value * MOTOR_INCREMENT);
  ...
  DebugMsgs.debug().print(_motor1.label).print(" Speed = ").println(_motor1.motorEncoderInfo->speed);
  DebugMsgs.debug().print(_motor2.label).print(" Speed = ").println(_motor2.motorEncoderInfo->speed);
